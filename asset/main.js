const data = [
  ...
  // ======Haupt Section=========
  const contentShoes = document.querySelector("#content-shoes");
  ...
  // ========= shoes div========
      ...
      data.forEach((item) => {
        ...
        imageElem.src=item.img;
        shoeDiv.appendChild(imageElem);
        shoeDiv.appendChild(pElem);
        shoeDiv.appendChild(priceElem);
        pElem.textContent = item.name;
        priceElem.textContent = item.price;
        pElem.addEventListener("click", (event) => {
          checkIndex(event.target.value, index);
        });
        shoeDiv.appendChild(pElem);
        contentShoes.appendChild(shoeDiv);
      });
